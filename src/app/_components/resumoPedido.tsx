...
export default function ResumoPedido({ pedido, setPedido }: ResumoPedidoProps) {
    ...
    return (
        <div className="flex flex-col rounded-lg border border-border bg-card p-4 shadow-md">
            <h1 className="mb-3 text-lg font-semibold">Resumo do Pedido</h1>
            ...
            <div className="mt-2 max-h-60 overflow-y-auto rounded border border-border p-2">
                <h1 className="text-lg font-semibold">Produtos</h1>
                <div className="flex flex-col gap-2 ">
                    {pedido.produtos.map((produto) => (
                        <div key={produto.id} className="mb-2 flex flex-col gap-2 rounded border border-border/60 bg-muted/40 p-2 sm:flex-row sm:items-center sm:justify-between">
                            <a className="break-words sm:max-w-[55%]">{produto.nome}</a>
                            <a className="shrink-0">{"R$ " + produto.valor}</a>
                            <button className="flex h-10 w-full items-center justify-center rounded bg-destructive font-bold text-destructive-foreground transition hover:bg-destructive/90 sm:w-10" onClick={() => RemoveProduto(produto.id)}>
